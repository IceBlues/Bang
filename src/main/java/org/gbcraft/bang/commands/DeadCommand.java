...
public class DeadCommand extends MFCommand {
    private final static Map<String, OfflinePlayer> containers = new HashMap<>();
    public DeadCommand(Bang plugin, CommandSender sender, String[] args) {
        super(plugin, sender, args);
    }

    public static void init(JavaPlugin plugin){
        List<String> players = ConfigHelper.getPlayersList("dead", plugin);
        players.forEach(p -> {
    ...
    public boolean run() {
        if (args.length >= 2) {
            ...
            plugin.sendMessage(sender, "info.command.dead");
        }
        return true;
    }
    ...
    public static void save(JavaPlugin plugin){
        ...
        ConfigHelper.savePlayersList("dead", list, plugin);
    }
